...
class OptimN2N:
  def __init__(self, loss_fn, model_update_params,
               lr=[1],
               iters=20,
  ...
  def forward(self, input, verbose=False):
    ...
    return self.forward_mom(input, verbose)
  ...
  def forward_mom(self, input, verbose=False):
    ...
    for k in range(self.iters):
      ...
      if self.max_grad_norm > 0:
        for j in range(len(input_grad_k)):
            self.clip_grad_norm([input_grad_k[j].data], self.max_grad_norm)
      ...
      lr_k_list = [self.lr for _ in range(len(input))]
  ...
  def backward_mom(self, grad_output, verbose=False):
    ...
    for k in reversed(range(rev_iters)):
      lr_k_list = [self.lr for _ in range(len(grad_output))]
