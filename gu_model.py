...
class Decoder(nn.Module):
    ...
    def __init__(self, field, args, causal=True,
        ...
        self.out = nn.Sequential(nn.Linear(args.d_model, len(field.vocab)), nn.LogSoftmax(-1))
    ...
    def forward(self, x, encoding, mask_src=None, mask_trg=None, input_embeddings=False, feedback=None, positions=None):
        ...
        if not input_embeddings:  # compute input embeddings
            if x.ndimension() == 2:
                x = F.embedding(x, self.out[0].weight * math.sqrt(self.d_model))
            elif x.ndimension() == 3:  # softmax relaxiation
                x = x @ self.out[0].weight * math.sqrt(self.d_model)  # batch x len x embed_size
...
class Fertility(nn.Module):
    ...
    def transform(fertilities, mask):
        ...
        source_indices = source_indices * mask.long() - (1-mask.long())
        ...
        assert (new_mask.sum().item() - fertilities.sum().item() == 0), '??'
...
class Transformer(nn.Module):
    ...
    def prepare_masks(self, inputs, is_trg=False):
        if not is_trg:
            if inputs.ndimension() == 2:
                masks = (inputs.data != self.srcfield.vocab.stoi['<blank>']).float()
            else:
                masks = (inputs.data[:, :, self.srcfield.vocab.stoi['<blank>']] != 1).float()
        else:
            if inputs.ndimension() == 2:
                masks = (inputs.data != self.field.vocab.stoi['<blank>']).float()
            else:
                masks = (inputs.data[:, :, self.field.vocab.stoi['<blank>']] != 1).float()
        return masks
